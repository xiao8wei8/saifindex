...
const App = () => {
    ...
    const getHeatMapsData = (number: number) => {
        ...
        if(number === 1){
            ...
            hs300Data = hs300.slice((number-1)*pageNum+1, number*pageNum);
        }
    ...
    return (
        <div id="chartContainer">
            {isShowStock ? (
                <div >
                    <div ><span style={{fontSize:"1.2em",fontWeight:"bold"}}>{name}-{value} </span><div className="right" ><Button type="primary" icon={<CloseOutlined onPointerEnterCapture={undefined} onPointerLeaveCapture={undefined} /> } onClick={onClose}>Close</Button></div></div>
                    <div>
                    <StockChart
                ...
                <div>
                <HeatMaps options={heatMapsOptions} highcharts={Highcharts2} />
                <div  className="right">
                {/* <Pagination defaultCurrent={1} total={15}  onChange={onChange} showSizeChanger={false}/> */}
                <Pagination defaultCurrent={1} total={pageLen} defaultPageSize={pageNum} onChange={onChange} showSizeChanger={false}/>
                </div>
