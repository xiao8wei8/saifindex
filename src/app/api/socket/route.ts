...
export async function POST(request: NextRequest, response: NextResponse) {
    ...
    let message = "",
        isforce = false,socketid="";

    try {
        const queryData = await request.json();
        console.log("[queryData]", queryData);
        message = queryData.message;
        isforce = queryData.isforce;
        socketid = queryData.socketid;
       
        logicfn(message, isforce,socketid);
    } catch (error: any) {
...
const logicfn = async (message: string, isforce: boolean,socketid: string) => {
    console.log("[logicfn]")
